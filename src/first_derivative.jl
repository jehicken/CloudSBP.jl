...
function compatible_surf_quad!(surf_wts, surf_pts, cell::Cell{Data, Dim, T, L},
                               xc, degree, E, H_cell) where {Data, Dim, T, L}

    num_basis = binomial(Dim + degree, Dim)
    num_nodes = size(xc,2)
    num_quad = size(surf_wts,2)
    xref = cell.data.xref 
    dx = cell.data.dx
    ...
    surf_trans = zero(surf_pts)
    for I in CartesianIndices(surf_pts)
        surf_trans[I] = (surf_pts[I] - xref[I[1]])/dx[I[1]] - 0.5 
    end

    # evaluate the polynomial basis at the nodes and surface quadrature points 
    ...
    poly_basis!(Vq, degree, surf_trans, workq, Val(Dim))

    # solve constraint in a minimum norm sense, and enforce compatibility
    num_eqns = div((num_basis + 1)*num_basis, 2)
    num_var = num_quad
    A = zeros(num_eqns, num_var)
    ...
    for i = 1:num_basis 
        for j = 1:i
            A[ptr,:] = Vq[:,i].*Vq[:,j]
            for d = 1:Dim
                B[ptr,d] = dot(dV[:,i,d], H_cell.*V[:,j]) + 
                    dot(dV[:,j,d], H_cell.*V[:,i])
                B[ptr,d] /= dx[d]
                B[ptr,d] -= dot(V[:,i], E[:,:,d] * V[:,j])
            end
            ptr += 1
        end
    end
    @assert( ptr - 1 == num_eqns )
    w = zeros(num_quad)
    for d = 1:Dim 
        # solve the system 
        B[:,d] -= A*surf_wts[d,:]
