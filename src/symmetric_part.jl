...
function cell_symmetric_part(cell::Cell{Data, Dim, T, L}, xc, degree, levset;
    ...
    if length(surf_wts) == 0
        # the cell was not actually cut, so nothing left to do but check
        for dir = 1:Dim
...
"""
    E = cell_symmetric_part(cell, faces, xc, degree)

Returns the symmetric part of the first-derivative SBP operator for the uncut 
cell `cell`.  The point cloud associated with `cell` is `xc`, and the boundary 
operator is `2*degree` exact for boundary integrals.  This variant loops over
the `faces` of the cell, which is not necessary for uncut cells but it is
necessary for cut cells.

**Note**: This version recomputes the 1D quadrature rule each time, and involves
several allocations.
"""
function cell_symmetric_part(cell::Cell{Data, Dim, T, L},
                             faces::Vector{Face{Dim, T, Cell{Data, Dim, T, L}}},
    ...
    E = zeros(num_nodes, num_nodes, Dim)
    for face in faces
        @assert( !is_cut(face), "face should not be cut for this method")
        rect = face.boundary
        dir = abs(face.dir)
        if abs(rect.origin[dir] - cell.boundary.origin[dir]) < 1e-14
            # interfaces always have positive dir, so need to account for that
            dir *= -1
        end
        face_quadrature!(xq_face, wq_face, rect, x1d, w1d, abs(dir))
        build_interpolation!(interp, degree, xc, xq_face, xref, dx)
...
"""
    E = cell_symmetric_part(cell, faces, xc, degree, levset [,fit_degree=degree])

Returns the symmetric part of the first-derivative SBP operator for the 
_possibly_ cut cell `cell`.  The point cloud associated with `cell` is `xc`, 
and the boundary operator uses `2*degree` exact quadrature.  The `fit_degree`
input indicates the degree of the Bernstein polynomial used by Algoim to
approximate the level set.  This variant loops over the `faces` of the cell,
which is not necessary for uncut cells but it is necessary for cut cells.
"""
function cell_symmetric_part(cell::Cell{Data, Dim, T, L}, 
                             faces::Vector{Face{Dim, T, Cell{Data, Dim, T, L}}},
                             xc, degree, levset; fit_degree::Int=degree
                             ) where {Data, Dim, T, L}
    @assert( length(cell.data.dx) > 0, "cell.data.dx is empty")
    @assert( length(cell.data.xref) > 0, "cell.data.xref is empty")
    num_nodes = size(xc,2)
    ...
    for face in faces
        ...
        wq_face, xq_face = cut_face_quad(rect, abs(dir), levset, degree+1,
                                         fit_degree=fit_degree)
        interp = zeros(length(wq_face), num_nodes)
        build_interpolation!(interp, degree, xc, xq_face, xref, dx)
        for i in axes(interp,2)
    ...
    if length(surf_wts) == 0
        # the cell was not actually cut, so nothing left to do but check
        for dir = 1:Dim
            #@assert( abs(sumE[dir]) < 10^degree*1e-12,
            #        "geo. cons. law failed (1)" )
            if abs(sumE[dir]) > 10^degree*1e-12
                println("abs(sumE[",dir,"] = ",abs(sumE[dir]))
            end
        end
        return E
...
nodes where the degrees of freedom are stored.  `ifaces` is an array of
intefaces and `bfaces` is an array of boundary faces corresponding to `root`.
`levset` is a function that defines the immersed geomtry, if any.  The symmetric
...
"""
function symmetric_operator(root::Cell{Data, Dim, T, L}, ifaces, bfaces, xc,
                            levset, degree; fit_degree::Int=degree
                            ) where {Data, Dim, T, L}
